...
(async () => {
  ...
  for (const box of boxes) {
    ...
    for (const email of emails) {
      ...
      // Zelfde food-heuristiek als de dagelijkse scan (geen telecom/utility/software/wijn)
      if (!lijktFoodLeverancier(email)) continue;
      const subject = email.subject || '';
      if (!found[addr]) found[addr] = { naam: afzenderNaam(email), laatste_onderwerp: subject.slice(0, 140), aantal: 0 };
      found[addr].aantal++;
    }
  }
